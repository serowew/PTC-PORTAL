...
import "../../../styles/announcementCreateP.css";
...
export default function CreateAnnouncement({
  ...
  const actorUserId = Number(session?.user_id);
  const actorRoleId = Number(session?.role_id);
  ...
  const [publishDate, setPublishDate] = useState(getLocalToday);
  ...
  const returnToAnnouncementList = () => {
    if (onCancel) {
      onCancel();
      return;
    }

    navigate("/programhead/announcementprog");
  };
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!Number.isInteger(actorUserId) || actorUserId <= 0) {
      setError("Unable to identify the logged-in Program Head user.");
      return;
    }

    if (!Number.isInteger(actorRoleId) || actorRoleId !== 3) {
      setError("Your Program Head role information is invalid. Please log in again.");
      return;
    }
    ...
    try {
      ...
      /*
        Attachment is optional. A failed file upload must not prevent the
        announcement itself from being created.

        We still keep the server error so the Program Head knows that the
      ...
      */
      if (selectedFile) {
        try {
          ...
          /*
            Authentication errors should still stop the request because the
            announcement POST would also fail with the same expired session.
          */
          if (
          ...
          uploadWarning =
            `The announcement will be created without the attachment. ${message}`;

          setAttachmentWarning(uploadWarning);
          fileId = null;

          console.warn(
            "PROGRAM HEAD ATTACHMENT UPLOAD SKIPPED:",
            uploadError,
          );
        }
      }
      ...
      console.log("CREATE PROGRAM HEAD ANNOUNCEMENT:", announcementData);
      ...
      if (response.status === 403) {
        throw new Error(
          data?.message ||
            data?.error ||
            "Program Head creation is not enabled by the announcement backend.",
        );
      }
  ...
  const formReady =
    ...
    recipients.length > 0 &&
    Boolean(publishDate) &&
    (!expiryDate || expiryDate >= publishDate);
  ...
  return (
    <DashboardLayout>
      <main className="programhead-announcement-create">
        <header className="programhead-announcement-create__hero">
          <div className="programhead-announcement-create__hero-copy">
            <div className="programhead-announcement-create__eyebrow">
              <span className="programhead-announcement-create__eyebrow-icon">
          ...
          <div className="programhead-announcement-create__hero-actions">
            <button
              type="button"
              className="programhead-announcement-create__button programhead-announcement-create__button--secondary"
              onClick={returnToAnnouncementList}
              disabled={loading}
            >
              <ArrowLeft size={17} />
              Announcements
        ...
        {error && (
          <div className="programhead-announcement-create__notice" role="alert">
            <AlertCircle size={19} />
            <div>
        ...
        {attachmentWarning && (
          <div
            className="programhead-announcement-create__notice"
            role="status"
          >
            <AlertCircle size={19} />

            <div>
              <strong>Attachment could not be uploaded</strong>
              <p>
                {attachmentWarning}
              </p>
            </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="programhead-announcement-create__form-grid">
            <div className="programhead-announcement-create__main-column">
              <section className="programhead-announcement-create__panel">
                <div className="programhead-announcement-create__panel-header">
                  <span className="programhead-announcement-create__panel-icon">
                    <FileText size={19} />
                  </span>
                  <div>
                    <span className="programhead-announcement-create__section-kicker">
                ...
                <div className="programhead-announcement-create__fields">
                  <label className="programhead-announcement-create__field">
                    <span>
                      Title <em>*</em>
                    </span>
                    <input
              ...
              <section className="programhead-announcement-create__panel">
                <div className="programhead-announcement-create__panel-header programhead-announcement-create__panel-header--split">
                  <div className="programhead-announcement-create__panel-heading-group">
                    <span className="programhead-announcement-create__panel-icon">
                      <UsersRound size={19} />
                    </span>
                    <div>
                  ...
                  <span className="programhead-announcement-create__count-badge">
                    {recipients.length} selected
                  </span>
                </div>

                <div className="programhead-announcement-create__recipient-grid">
                  {RECIPIENT_ROLES.map((role) => {
                    const selected = recipients.includes(role.role_id);

                    return (
                      <label
                        className={`programhead-announcement-create__recipient-card ${
                          selected
                            ? "programhead-announcement-create__recipient-card--selected"
                            : ""
                        }`}
                        key={role.role_id}
                      >
                        <input
                          ...
                          disabled={loading}
                        />
                        <span className="programhead-announcement-create__recipient-check">
                          {selected && <Check size={14} strokeWidth={2.5} />}
                        </span>
                        <span className="programhead-announcement-create__recipient-name">
                          {role.role_name}
                        </span>
                      </label>
                    );
                ...
                {selectedRecipientNames.length > 0 && (
                  <div className="programhead-announcement-create__audience-summary">
                    <UsersRound size={16} />
                    <span>Audience:</span>
              ...
              <section className="programhead-announcement-create__panel">
                ...
                <div className="programhead-announcement-create__upload-box">
                  <span className="programhead-announcement-create__upload-icon">
                    <FilePlus2 size={22} />
                  </span>
                  <div className="programhead-announcement-create__upload-copy">
                ...
                {selectedFile && (
                  <div className="programhead-announcement-create__selected-file">
                    <span className="programhead-announcement-create__selected-file-icon">
                      <FileText size={18} />
                    </span>
                    <div>
            ...
            <aside className="programhead-announcement-create__side-column">
              <section className="programhead-announcement-create__panel programhead-announcement-create__panel--compact">
                <div className="programhead-announcement-create__panel-header">
                  <span className="programhead-announcement-create__panel-icon">
                ...
                <div className="programhead-announcement-create__fields">
                  ...
                  <div
                    className={`programhead-announcement-create__status-preview ${
                      isActive
                        ? "programhead-announcement-create__status-preview--active"
                        : "programhead-announcement-create__status-preview--inactive"
                    }`}
                  >
                    {isActive ? (
                  ...
                  <label className="programhead-announcement-create__field">
                    ...
                    <input
                      ...
                      disabled={loading}
                    />
                    <small className="programhead-announcement-create__field-note">
              ...
              <section className="programhead-announcement-create__checklist-panel">
                <div className="programhead-announcement-create__checklist-header">
                  <span>
                    <ShieldCheck size={18} />
                  </span>
                  <div>
                ...
                <div className="programhead-announcement-create__checklist">
              ...
              <section className="programhead-announcement-create__save-panel">
                <div className="programhead-announcement-create__save-icon">
                  <Send size={20} />
                </div>
                <div>
                  <span className="programhead-announcement-create__section-kicker">
                ...
                <div className="programhead-announcement-create__save-actions">
                  <button
                    type="button"
                    className="programhead-announcement-create__button programhead-announcement-create__button--secondary"
                    onClick={returnToAnnouncementList}
                  ...
                  <button
                    type="submit"
                    className="programhead-announcement-create__button programhead-announcement-create__button--primary"
                    disabled={
                      loading ||
                      !authenticated ||
                  ...
                  >
                    {loading ? (
                      <>
                        <Loader2
                          size={17}
                          className="programhead-announcement-create__spin"
                        />
                        Creating...
                      </>
                    ) : (
